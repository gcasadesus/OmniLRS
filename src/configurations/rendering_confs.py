...
class FlaresConf:
    ...
    def __post_init__(self):
        ...
        assert type(self.aperture_rotation) is float, (
            "aperture_rotation must be a float"
        )
        assert type(self.sensor_diagonal) is float, "sensor_diagonal must be a float"
        assert type(self.sensor_aspect_ratio) is float, (
        ...
        assert self.aperture_rotation >= 0, (
            "aperture_rotation must be greater or equal to 0"
        )
        assert self.aperture_rotation <= 360, (
            "aperture_rotation must be smaller or equal to 360"
        )
...
class MotionBlurConf:
    ...
    def __post_init__(self):
        ...
        assert type(self.max_blur_diameter_fraction) is float, (
            "max_blur_diameter_fraction must be a float"
        )
        assert type(self.exposure_fraction) is float, (
            "exposure_fraction must be a float"
        )
        assert type(self.num_samples) is int, "num_samples must be an integer"

        assert self.max_blur_diameter_fraction > 0, (
            "max_blur_diameter_fraction must be greater than 0"
        )
...
class ChromaticAberrationsConf:
    ...
    def __post_init__(self):
        ...
        assert all([type(i) is float for i in self.strength]), (
            "strength must be a tuple of floats"
        )
        assert len(self.strength) == 3, "strength must be a tuple of 3 floats"
        assert type(self.model) is tuple, "model must be a tuple"
        assert all([type(i) is str for i in self.model]), (
            "model must be a tuple of strings"
        )
        ...
        assert all([i in ["Radial", "Barrel"] for i in self.model]), (
            "model must be a tuple of 'Radial' or 'Barrel'"
        )
        assert all([i <= 1 and i >= -1 for i in self.strength]), (
...
class RendererConf:
    ...
    livestream: int = dataclasses.field(default=0)
    livestream_public_ip: str = dataclasses.field(default="127.0.0.1")
    livestream_port: int = dataclasses.field(default=49100)
    ...
    def __post_init__(self):
        ...
        assert type(self.livestream) is int, "livestream must be an integer"
        assert type(self.livestream_public_ip) is str, "livestream_public_ip must be a string"
        assert type(self.livestream_port) is int, "livestream_port must be an integer"

        assert self.samples_per_pixel_per_frame > 0, (
            "samples_per_pixel_per_frame must be greater than 0"
        )
        assert self.max_bounces > 0, "max_bounces must be greater than 0"
        assert self.max_specular_transmission_bounces > 0, (
            "max_specular_transmission_bounces must be greater than 0"
        )
        assert self.max_volume_bounces > 0, "max_volume_bounces must be greater than 0"
        assert self.subdiv_refinement_level >= 0, (
            "subdiv_refinement_level must be greater or equal to 0"
        )
        assert self.livestream in [0, 1, 2, 3], (
            "livestream must be 0, 1 (websocket), 2 (webrtc), or 3 (native)"
        )
